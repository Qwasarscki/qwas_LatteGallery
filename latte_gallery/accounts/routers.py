...
from latte_gallery.security.dependencies import (AuthenticatedAccount, AuthorizedAccount,
                                                 create_access_token, authenticate_user_by_token)
...
async def authenticate_by_cookie(request: Request, account_service: AccountServiceDep, session: SessionDep):
    ...
    account = await authenticate_user_by_token(cookie_token, account_service, session)
    if not account:
...
async def update_account_by_id(
        ID: PositiveInt, Body: AccountUpdateSchema, AccountService: AccountServiceDep, Session: SessionDep
) -> AccountSchema:
    return await AccountService.update_by_id(ID, Body, Session)